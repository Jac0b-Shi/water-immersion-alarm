 ...
 * Version            : V1.3.0
 * Date               : 2026/03/31
 * Description        : 浸水检测报警系统主程序
 *                      当前维护目标为 CH32V208WBU6
 *                      提供浸水检测、状态管理和企业微信告警发送
 ...
 *
 * 当前维护范围：
 * - ADC 检测浸水传感器模拟量，默认浸水阈值 1000mV
 * - 双 LED 指示系统状态（PB0=正常，PB1=报警，低电平点亮）
 * - BC260 NB-IoT 与 CH32V208 内置以太网两种通信路径
 * - 通过 HTTP 代理转发到企业微信 Webhook
 * - 状态去抖、周期检测和告警发送逻辑
 *******************************************************************************/

/*
 * 硬件与配置摘要（CH32V208WBU6）：
 *
 * 传感器输入：
 * - PA0：浸水传感器模拟量输入
 ...
 * 内置以太网：
 * - 使用内置PHY，需连接RJ45网口
 * - 不支持 SSL/TLS，需通过 HTTP 代理转发 HTTPS 请求
 *
 * BC260：
 * - PA2：USART2_TX
 * - PA3：USART2_RX
 ...
 *
 * 配置说明：
 * - 修改 config.env 后运行 generate_config.py 生成 User/config.h
 * - 通信开关在 config.h 中控制
 * - 当前不维护 ESP8266 路径
 */
...
#include "string.h"
#include "stdio.h"
#include "config.h"  // 由 generate_config.py 生成的项目配置

#if ENABLE_ESP8266
#error "ESP8266 support is deprecated and no longer maintained. Use BC260 or Ethernet instead."
#endif
...
char BC260_IMEI[16] = {0};                               // 设备IMEI (15位+结束符)
#endif
 ...
 * @return  1-成功 0-失败
 */
uint8_t BC260_GetIMEI(void)
{
    char* imei_start;
    char* imei_end;
    
    printf("[BC260] Getting IMEI...\r\n");
    
    // 发送AT+CGSN=1查询IMEI
    if(!BC260_SendCommand("AT+CGSN=1", "+CGSN:", 5000))
    {
        printf("[BC260] Failed to get IMEI!\r\n");
        return 0;
    }
    
    // 解析IMEI，格式: +CGSN: 490154203237511\r\nOK\r\n
    imei_start = strstr((char*)BC260_rx_buffer, "+CGSN: ");
    if(imei_start)
    {
        imei_start += 7;  // 跳过 "+CGSN: "
        
        // 查找IMEI结束位置（找到\r或空格）
        imei_end = imei_start;
        while(*imei_end && *imei_end != '\r' && *imei_end != '\n' && *imei_end != ' ')
        {
            imei_end++;
        }
        
        int imei_len = imei_end - imei_start;
        if(imei_len >= 14 && imei_len <= 17)  // IMEI通常是15位
        {
            strncpy(BC260_IMEI, imei_start, imei_len);
            BC260_IMEI[imei_len] = '\0';
            printf("[BC260] IMEI: %s\r\n", BC260_IMEI);
            return 1;
        }
    }
    
    printf("[BC260] Failed to parse IMEI!\r\n");
    return 0;
}
 ...
 *
 * 带密钥的数据包格式:
 *   [密钥字符串] + [3字节二进制数据的HEX编码(6字节)]
 *   例如: "MYPASS" + "089209" = "MYPASS089209"
 *
...
uint8_t BC260_SendAlertUDP(uint8_t msg_type, uint8_t water_status, uint8_t flags, uint16_t adc_value)
{
    ...
    // 构建带设备ID和密钥的完整数据包
    // 格式: [IMEI:] + [密钥] + [HEX数据]
    // 例如: "869999051234567:KEY123089209"
    char full_payload[128];
    const char* secret_key = UDP_SECRET_KEY;
    
    // 检查是否有IMEI和密钥
    int has_imei = (BC260_IMEI[0] != '\0');
    int has_key = (secret_key && strlen(secret_key) > 0);
    
    if(has_imei && has_key)
    {
        // IMEI + 密钥 + HEX数据
        sprintf(full_payload, "%s:%s%s", BC260_IMEI, secret_key, hex_payload);
        printf("[BC260_UDP] Using device ID (IMEI) + authentication key\r\n");
    }
    else if(has_imei)
    {
        // 只有IMEI，无密钥
        sprintf(full_payload, "%s:%s", BC260_IMEI, hex_payload);
        printf("[BC260_UDP] Using device ID (IMEI), WARNING: No authentication key!\r\n");
    }
    else if(has_key)
    {
        // 只有密钥，无IMEI
        sprintf(full_payload, "%s%s", secret_key, hex_payload);
        printf("[BC260_UDP] Using authentication key (no device ID)\r\n");
    }
    else
    {
        // 都没有
        strcpy(full_payload, hex_payload);
        printf("[BC260_UDP] WARNING: No device ID or authentication key!\r\n");
    }
    
    printf("[BC260_UDP] Encoding payload: type=%d, water=%d, flags=%d, adc=%d\r\n",
           msg_type, water_status, flags, adc_value);
    printf("[BC260_UDP] Device: %s\r\n", has_imei ? BC260_IMEI : "unknown");
    printf("[BC260_UDP] Full payload: %s (%d bytes)\r\n", full_payload, (int)strlen(full_payload));
    ...
    for(send_attempt = 0; send_attempt < 2; send_attempt++)
    {
        ...
        sprintf(cmd, "AT+QISEND=0,%d", (int)strlen(full_payload));
    ...
    // 6. 发送完整数据包（密钥+HEX数据）+ Ctrl-Z
    printf("[BC260_UDP] Sending full payload: %s\r\n", full_payload);
    char* p = full_payload;
    while(*p)
    {
        while(USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET);
