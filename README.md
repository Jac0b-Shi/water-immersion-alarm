...
ULTRASONIC_FILTER_TRIM_PERCENT=25
```

其中 `ULTRASONIC_SAMPLING_*` 用于控制板端主动测距节奏：

- 先空闲 `ULTRASONIC_SAMPLING_IDLE_SECONDS`
- 再进入最多 `ULTRASONIC_SAMPLING_BURST_DURATION_SECONDS` 的突发窗口
- 窗口内每隔 `ULTRASONIC_SAMPLING_BURST_INTERVAL_MS` ms 触发一次测距
- 窗口内忽略无效测距值，窗口结束后对有效样本按 `ULTRASONIC_FILTER_TRIM_PERCENT` 去掉低端和高端极值，再取平均值作为本轮测距结果

如果希望退回旧的“按滤波窗口均匀采样”逻辑，可将 `ULTRASONIC_SAMPLING_BURST_DURATION_SECONDS=0`。
